...
export class UserService {
  ...
  sendInvite(data): Observable<any> {
    return this.http.post<IBaseResponse<any>>(`${API_BASE_URL}api/invite-member/`, data);
  }

  getUsers(data): Observable<any> {
    return this.http.get<IBaseResponse<any>>(`${API_BASE_URL}api/user/`, { params: data });
  }

  getPendingUsers(data): Observable<any> {
    return this.http.get<IBaseResponse<any>>(`${API_BASE_URL}api/company-pending-invitation/${data.company}`, {});
  }

  getGroup(): Observable<any> {
    const queryString = '?is_user_specific=false&ordering=-is_public,name';
    return this.http.get<IBaseResponse<any>>(`${API_BASE_URL}api/group/${queryString}`, {});
  }
  ...
  updatePendingUser(id, data): Observable<any> {
    return this.http.patch<IBaseResponse<any>>(`${API_BASE_URL}api/update-invited-member-group/${id}/`, data);
  }

  userDetails(data): Observable<any> {
    return this.http.get<IBaseResponse<any>>(`${API_BASE_URL}api/user/me/`, data);
  }
  ...
  reSendInvite(id): Observable<any> {
    return this.http.post<IBaseResponse<any>>(`${API_BASE_URL}api/resend-pending-invite/${id}`, {});
  }

  getUserWorkgroup(limit: number, offset: number, ordering): Observable<any> {
    let queryString = '?limit=' + limit + '&offset=' + offset;
    if (ordering) {
      queryString = queryString + '&ordering=' + ordering;
    }
    return this.http.get<IBaseResponse<any>>(`${API_BASE_URL}projects/api/user_workgroup/${queryString}`);
  }
  ...
  listCompanyUserNotifications(userId: number, modelType: string): Observable<any> {
    ...
    return this.http.get<IBaseResponse<any>>(`${API_BASE_URL}notifications/api/company_user_notification_settings/?user_id=${userId}&model_type=${modelType}`);
  }
  ...
  userPermission(): Observable<any> {
    ...
    return this.http
      .get<IBaseResponse<any>>(`${API_BASE_URL}api/my-permission/?limit=100`)
      .pipe(
        map((data: any) => {
